...
do
  ...
  ((port=$REDIS_PORT_BASE+$index))
  ssh $server "sh -c \"cd ${CLUSTER_INSTALL_DIR}/$port; ${REDIS_DIR}/redis-server ./${CONFIG_FILE} > /dev/null 2>&1 &\""
  ((index=index+1))
done
...
cmd="$REDIS_DIR/redis-cli -c -h ${first_server} -p ${REDIS_PORT_BASE} cluster nodes"
$cmd | sort -k9 -n
